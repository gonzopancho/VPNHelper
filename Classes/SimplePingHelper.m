...
@synthesize simplePing, target, sel, index;
...
+ (void)ping:(NSString*)address target:(id)target sel:(SEL)sel {
	...
	[[[[SimplePingHelper alloc] initWithAddress:address target:target sel:sel index:0] autorelease] go];
}
...
- (void)successPing {
	[self killPing];
	[target performSelector:sel withObject:[NSNumber numberWithInt:index]];
}

- (void)failPing:(NSString*)reason {
	[self killPing];
	[target performSelector:sel withObject:[NSNumber numberWithInt:(-index - 1)]];
}
